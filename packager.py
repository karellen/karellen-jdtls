...
def extract_product(product_dir: Path, platform: str) -> Path:
    ...
    if platform.startswith("linux-gtk-x86_64"):
        return product_dir / "linux" / "gtk" / "x86_64"
    elif platform.startswith("linux-gtk-aarch64"):
        return product_dir / "linux" / "gtk" / "aarch64"
    elif platform.startswith("macosx-cocoa-x86_64"):
        return product_dir / "macosx" / "cocoa" / "x86_64"
    elif platform.startswith("macosx-cocoa-aarch64"):
        return product_dir / "macosx" / "cocoa" / "aarch64"
    elif platform.startswith("win32-x86_64"):
        return product_dir / "win32" / "win32" / "x86_64"
    else:
...
def create_python_package(staging_dir: Path):
    ...
    (pkg_dir / "launcher.py").write_text(LAUNCHER_PY)
...
def create_build_files(staging_dir: Path, version: str):
    ...
    (staging_dir / "setup.py").write_text(setup_py)
    ...
    (staging_dir / "setup.cfg").write_text(setup_cfg)
